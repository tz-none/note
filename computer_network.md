...
- 无确认无连接——适用于实时通信，绝大多数局域网采用，以太网
- 有确认无连接——使用于不可靠信道，WiFi
- 有确认有链接——需要进行连接，为网络层进程提供可靠的位流

### 成帧方法

字节计数法：数据帧首位表示数据帧长

标志字节填充法：使用特殊字符作为帧头和帧尾，起始字符DLE STX，结束字符DLE ETX，传输数据中有特殊字符时，在其前插入DLE。

标志位填充法：帧的起始和结束都使用一个特殊的位串“01111110”，称为标记。在传输的数据中每5个1插入一个0（透明传输策略：0插入法）。

物理层编码违例法：借用一些违法编码序列来定界帧的起始与终止。
...
差错检测和纠错：利用检错码和纠错码（添加冗余位）
> code word(码字) = 数据位 + 冗余位
...
- 结合ARQ和FEC的方式
- 传输错码位数较少且接收端能纠正是，自动纠正错误
- ……

常用检错码：

- 奇偶校验：通过增加冗余位是码字中“1”的个数保持奇数或偶数的编码方式。简单但漏检率较高。
- 循环冗余校验码（CRC）
- 校验和：将序列分为段，计算他们的和，若计算是有进位，则将进位加到最后的校验和中，并将校验和与数据一起发送。接收端重新计算校验和进行比较。

纠错码：

- 海明码
  - 冗余位数：满足2^r >= m+r+1的最小r值
  - 冗余位定位：2的n次方处
  - 每个冗余位都是一组数据位的奇偶校验码。
- 二进制卷积码

流量控制：发送方和接收方速率的匹配，通常用窗口协议实现。
